...
class data_generator(object):
    ...
    def next_sample(self):
        ...
        for sample_index in self.sample_list[rng_idx[0]: rng_idx[1]]:
            ...
            in_data = data
            to_pred =  torch.from_numpy(in_data['pred_mask']).to(bool)
            if not to_pred.any():
                continue
            obs_traj.append(torch.stack(in_data['pre_motion_3D'])[to_pred])
            fut_traj.append(torch.stack(in_data['fut_motion_3D'])[to_pred])
