    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, radius, compute_local_homo=False):
        ...
        for agent_idx in range(len(padded_map)):
            ...
            if compute_local_homo:
                ...
                #     torch.cat([torch.from_numpy(all_pixel_local), torch.ones((len(all_pixel_local), 1))], dim=1),
                #     torch.transpose(torch.from_numpy(h).float(), 1, 0))
                # back_wc /= back_wc[:, 2].unsqueeze(1)

                # diff = torch.from_numpy(all_traj) - back_wc[:,:2]
