...
from micro_diffusion import Solver as micro_diffusion_solver
from eval_guided import Solver as eval_guided_solver
...
def create_parser():
    ...
    parser.add_argument('--model_name', default='micro', type=str,
                        help='model name: one of [lg_ae, lg_cvae, sg_net, micro, micro_diffuse]')
    ...
    parser.add_argument('--alternative_lg', default=-1, type=int)
    ...
    parser.add_argument('--use_sg_net', default=False, type=bool)
...
def main(args):
    ...
    if args.ckpt_load_iter == args.max_iter:
        ...
        # change this to change which micro-stage model to use
        # solver = eval_solver(args)
        # solver = eval_diffuse_solver(args)
        # solver = eval_jux_solver(args)
        solver = eval_guided_solver(args)
        ...
        elif args.model_name =='ig_ae' or args.model_name =='ig_cvae':
            solver = lg_solver(args)
        elif args.model_name =='sg_net':
            solver = sg_solver(args)
        elif args.model_name =='micro': # MUSE-VAE micro-stage model
            solver = micro_solver(args)
        elif args.model_name =='micro_diffuse': # TrajDiffuse Model
            solver = micro_diffusion_solver(args)
        else:
