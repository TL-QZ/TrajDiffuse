...
class heatmap_generation(object):
    ...
    def create_nu_heatmap(self, local_ic, local_map, aug=False, route=False):
        """
        :param local_ic: (batch, seq_len, 2)
        :param local_map: (batch, height, width)

        :return: (batch, 4, height, width) if route and self.sg_idx is None
            3: [env+past, route, final goal]
        :return: (batch, 3, height, width) if self.sg_idx is None
            2: [env+past, final goal]
        :return: (batch, 3+len(self.sg_idx), height, width) if route and self.sg_idx is not None
            2+len(self.sg_idx): [env+past, route, short goals]
        :return: (batch, 2+len(self.sg_idx), height, width) if self.sg_idx is not None
            1+len(self.sg_idx): [env+past, short goals]
        """
        heatmaps=[]
        if route: route_maps = []
        ...
        for i in range(len(local_ic)):
            ...
            # route map
            if route:
                route_map = np.zeros_like(local_map[i])
                for t in range(self.obs_len-1,len(local_ic[i])-1):
                    rr, cc = line(local_ic[i][t,0], local_ic[i][t,1], local_ic[i][t+1,0], local_ic[i][t+1,1])
                    route_map[rr, cc] = 1
                route_map = ndimage.filters.gaussian_filter(route_map, sigma=1)*(1-local_map[i])
                route_map[route_map != 0] = 1
            ...
            if map_size < self.heatmap_size:
                ...
                all_heatmap.append(extended_map)
                if route:
                    empty_route_map = np.zeros((self.heatmap_size, self.heatmap_size))
                    empty_route_map[half-map_size//2:half+map_size//2, half-map_size//2:half+map_size//2] = route_map
                    all_heatmap.append(empty_route_map)
                ...
                all_heatmap.append(heatmap / heatmap.sum())
                if route:
                    route_map = cv2.resize(route_map, dsize=(self.heatmap_size, self.heatmap_size))
                    route_map[route_map != 0] = 1
                    all_heatmap.append(route_map)
        ...
        if route:
            if self.sg_idx is None:
                return heatmaps[:, :2], heatmaps[:, 2:3], heatmaps[:, 3:]
            else:
                return heatmaps[:, :2], heatmaps[:, 2:3], heatmaps[:, 3:], heatmaps[:, -1].unsqueeze(1)
        else:
            if self.sg_idx is None:
                return heatmaps[:, :2], heatmaps[:, 2:]
            else:
...
def ECFL(global_traj, map_info):
    '''
    global_traj: list of tensor with shape (batch_size X num_samples X pred_len X 2)
    map_info: map object
    '''
    ecfl = 0.0
    if len(global_traj.shape) == 3:
        global_traj = global_traj.unsqueeze(0)
        
    traj_array = global_traj.cpu().numpy()
    
    map_traj = map_info.to_map_points(traj_array).astype(int)
    map_nav = extract_nav_map(map_info)
    navmaps = [map_nav]*len(traj_array)
    ecfl += compute_ECFL(map_traj, navmaps)

    return ecfl


def extract_nav_map(map_info):
    map_raw = map_info.data
    map_nav = map_raw / 255
    map_nav[0][np.where(map_nav[1] != 0)] = 0.3
    map_nav[0][np.where(map_nav[2] != 0)] = 0.6
...
ValueBatch = namedtuple('ValueBatch', 'trajectories conditions map center scale')

# ====== Emperical Moving Average ======
