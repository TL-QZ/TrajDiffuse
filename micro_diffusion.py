...
RANDOM_ANGLES = np.array([np.pi/4, 2*np.pi / 4, 3*np.pi / 4, 4*np.pi / 4, 5*np.pi / 4, 6*np.pi / 4, 7*np.pi / 4])
def rotate_randomly(trajectory):
    '''
    Rotate the trajectory randomly
    trajectory: np.array of shape (n_timesteps, 2)
    '''
    angle = np.random.choice(RANDOM_ANGLES)
    rotation_matrix_T = np.array([[np.cos(angle), np.sin(angle)],
                                [-np.sin(angle), np.cos(angle)]])
    
    return np.dot(trajectory, rotation_matrix_T)
...
class Solver(object):
    ...
    def train(self):
        ...
        for iteration in tqdm(range(start_iter, self.max_iter + 1)):
            ...
            for b in range(batch_size):
                ...
                normed_temp = (trajectories[b]-map_size[0]/2)/(map_size[0]/2)
                if self.augment:
                    if random.uniform(0,1) < 0.15:
                        normed_temp = rotate_randomly(normed_temp)
                        normed_temp = torch.from_numpy(normed_temp).to(torch.float32)
                local_ic_st.append(normed_temp)
            ...
            batch = Batch(local_ic_st, condition)
    ...
    def validate(self, epoch:int, writer) -> None:
        ...
        for iteration in tqdm(range(start_iter, iter_per_epoch )):
            ...
            global_preds = []
            for b in range(batch_size):
                pred_local = samples.trajectories[b]*std_scale[b]+std_scale[b]
            ...
            global_de.extend(displacement_error(global_preds[self.obs_len:,:,:], fut_traj[:, :, :2], mode='raw'))
    ...
    def eval_batch(self, condition, std_scale, center, local_homo, fut_traj):
        batch_size = len(condition[0])
        samples = self.model(condition, verbose=False)
        
        pred_global = []
        for i in range(batch_size):
            pred_local = samples.trajectories[i]*std_scale[i]+center[i]
            back_wc = torch.matmul(
                torch.cat([pred_local, torch.ones((len(pred_local), 1)).to(self.device)], dim=1),
                torch.transpose(local_homo[i].float().to(self.device), 1, 0))
            back_wc /= back_wc[:, 2].unsqueeze(1)
            pred_global.append(back_wc[:,:2])
        pred_global = torch.stack(pred_global).transpose(0,1)
        ade, fde = [], []
        ade.append(displacement_error(
                pred_global[self.obs_len:,:,:], fut_traj[:, :, :2], mode='raw'
            )/self.pred_len)
        fde.append(final_displacement_error(
            pred_global[-1], fut_traj[-1, :, :2], mode='raw'
        ))
        ade = torch.stack(ade)
        fde = torch.stack(fde)
